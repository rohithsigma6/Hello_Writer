...
import { useAddCollaborator } from '@/features/file/screenplay/api/add-colobrator';
...
import { useDeleteCollaborator } from '@/features/file/screenplay/api/delete-collaborator';
...
// Debounced function to fetch users based on the search
// export const deferredGetUsers = debounce(
//   ({ search, updateOptions, fileId }) => {
//     post('/user/search', { search, fileId }).then(({ data }) => {
//       const mappedUsers = data.users?.map((user: { email: any; _id: any }) => ({
//         label: user.email,
//         value: user._id,
//       }));
//       updateOptions(mappedUsers);
//     });
//   },
//   200,
// );

// export const deferredUpdateTitle = debounce(
//   ({ title, fileId, updateFileTitle }) => {
//     post(`/file/${fileId}`, { title }).then(({ data }) => {
//       updateFileTitle(data?.title);
//       enqueueSnackbar('File title updated');
//     });
//   },
//   500,
// );
...
export function UserEmailSuggest({
  ...
  //   popupOpen,
  // } = useAutocomplete({
  //   id: 'search-user-emails',
  //   options: options,
  //   getOptionLabel: (option) => option.label,
  //   value,
  //   inputValue, // Manage input value separately
    ...
    <div className="w-full relative">
      ...
      >
        {groupedOptions.length > 0 && (
          <ul
            className="absolute text-sm box-border p-1.5 my-3 mx-0 min-w-[320px] rounded-xl overflow-auto max-h-[300px] bg-white border border-solid border-gray-200 shadow-lg"
            {...getListboxProps()}
          >
...
const ShareFilePopup = ({ open, setOpen }: PopupShareFileProps) => {
  const [invitee, setInvitee] = useState<SearchUser | null>(null);
  const { fileId } = useParams();
  ...
  const { mutate: deleteCollab } = useDeleteCollaborator({
    mutationConfig: {
      onSuccess: () => {},
    },
  });
...
const { mutate:deleteCollobrator } = useDeleteFileCollobrator();
  ...
  const handleAddEmail = (
    ...
    if (invitee) {
      ...
      console.log({
        fileId, // Assuming fileId is passed as a prop or from the state
  ...
  const handleAddCopyLink = (
    ...
    if (invitee) {
      ...
      // inviteUserFn({ fileId: file?._id, collaborators: [{ userId: newUserId, permissionType: permission }] });
    ...
    <Modal isOpen={open} setIsOpen={setOpen}>
      <div className="p-6">
        <div className="flex justify-between items-center">
          <h3 className="text-[19px] p-0 m-0 font-bold text-[#212131]">
            Share Script
          </h3>
          <button
        ...
        <div className="pt-4">
          <div>
            ...
            <div className="mt-8">
              <h3 className="text-[14px] text-[#212131] font-poppins font-semibold mb-4">
                People with Access
              </h3>
                ...
                emails?.map((email, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 w-[100px] min-w-[100px] max-w-[100px]">
                      <div className=" text-[12px] font-regular font-poppins text-[#212131] whitespace-nowrap overflow-hidden text-ellipsis ">
                        {email.permission}
                      </div>
                            ...
                            <Popover.Panel className="absolute left-1/2 z-[1000] mt-3 w-screen max-w-sm -translate-x-1/2 transform px-4 sm:px-0 lg:max-w-3xl">
                              <div className="absolute -top-4 left-36 bg-slate-800 rounded-3xl rounded-tr-none shadow-lg z-[1000] w-60">
                                <ul className="text-sm text-gray-300">
                                  {Object.entries(PERMISSIONS_MAP).map(
                                    ([key, value]) => (
                                      <li
                                        key={key}
        ...
        {linkName?.length ? (
          <div className="my-3">
            <h3 className="text-sm font-bold">General Access</h3>
            <div className="flex gap-3 mt-2">
              <button
                className="bg-primary-blue text-white px-8 py-3 rounded-2xl text-lg"
                onClick={() => {
                  // handleAddEmail(invitee1, inviteePermission1, setInvitee1, setShowEmailOptions1);
                }}
              >
                Restricted
              </button>
              <button
                className="bg-slate-100 text-slate-400 px-6 py-3 rounded-2xl text-lg"
                onClick={() => setShowEmailOptions1(false)}
              >
                Anyone with link
              </button>
