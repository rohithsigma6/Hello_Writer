...
import { useAddLocation } from '../api/location-data';
import { enqueueSnackbar } from 'notistack';
import Select from 'react-select'; 
import { Swiper, SwiperSlide } from 'swiper/react';
import 'swiper/swiper-bundle.css';
import { useDeleteLocationImage } from '../api/delete-location';
...
const [removeId, setRemoveId] = useState([])
  ...
  const { mutate } = useDeleteLocationImage();
  const handleChange = (
  ...
  const { fileId } = useParams<{ fileId: any }>();
  ...
  useEffect(() => {
    if (data) {
      ...
      setPrimaryIndex(data?.images?.findIndex(item => item.isPrimary));
    }
  }, [data]);
  ...
  const handleCreate = () => {
    ...
    files.forEach((file) => {
      formDataToSend.append('images', file); // Backend should expect 'images' as an array
    });
  
    // Append primary index
    formDataToSend.append('primaryImageIndex', primaryIndex !== null ? primaryIndex.toString() : '0');
  
    // Append type and fileId
    formDataToSend.append('type', selectedOption || '');
    formDataToSend.append('fileId', fileId);
  
    if(data?._id && removeId.length){
      mutate({id:data?._id, payload:{imageIds:removeId}}, {
        onSuccess: (response) => {
    ...
    addLocationMutation.mutate(
      ...
      },
      {
        ...
        onError: (error: any) => {
          enqueueSnackbar(`Error: ${error.message || 'Failed to add/update location'}`, { variant: 'error' });
        },
      }
    );
  };
  ...
  const handleFiles = (newFiles: FileList | null) => {
    ...
    validFiles.forEach((file) => {
      ...
      reader.onload = () => {
        ...
        if (primaryIndex === null && previewURLs.length === 0) {
          setPrimaryIndex(0); // Default the first image to primary if none selected
        }
      };
                ...
                <div className="relative w-full h-[300px]">
                  ...
                  <button
                    className={`absolute bottom-2 left-1/2 transform -translate-x-1/2 py-1 px-3 rounded-md ${
                      primaryIndex === index ? 'bg-green-600' : 'bg-[#653EFF]'
                    } text-white`}
