...
import WorkflowIllustrators from './UI/workflow-illustrators';

const WelcomeSceneCard = () => {
            ...
            <div className="bg-white rounded-[24px] px-[32px] pt-[40px] font-poppins ">
                <div className="flex xl:flex-row flex-col gap-6 items-start">
                    ...
                    <div className="xl:hidden">
                        <img
                            src={sceneCardData?.smIcon}
                            loading="lazy"
                            className="mx-auto object-contain"
                        />
                    </div>
                    <div className="flex flex-col" key={sceneCardData.tipsHeading}>
                        <section>
                            <p className="text-2xl font-semibold text-text-500">
                                {sceneCardData.heading}
                            </p>
                            <p className="mt-[16px] text-text-200 text-base">
                                {sceneCardData.paragrapgh}
                            </p>
                        </section>
                        <section className="mt-[24px]">
                            ...
                            <ul className="ml-2 mt-2">
                                {sceneCardData.list.map((listItem) => (
                        ...
                        <section className="gap-y-3">
                            <button
                                ...
                                onClick={() =>
                                    navigate(`/file/${fileId}/scenecards/template`)
                                }                            >
                                {sceneCardData.btnText}
                            </button>
                        </section>
                    </div>
                </div>
                                ...
                                <Disclosure.Button className="rounded-[16px] border mb-10">
                                    ...
                                    {sceneCardData && open &&
                                        <section className="flex flex-col gap-y-3 p-4 h-fit overflow-auto font-poppins">
                                            {sceneCardData?.tips.map((item, index) => (
                                                <Disclosure.Panel
                                                    key={index}
...
export default WelcomeSceneCard;
