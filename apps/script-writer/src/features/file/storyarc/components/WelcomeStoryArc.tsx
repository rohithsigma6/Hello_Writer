...
import { storyArcData } from '../data';
import { FiChevronDown, FiChevronUp } from 'react-icons/fi';

const WelcomeStoryArc = ({ setShowCreatePage }: { setShowCreatePage: (value: 'create') => void }) => {
  return (
    <>
          ...
          <div className="bg-white rounded-[24px] px-[32px] pt-[40px] font-poppins ">
            <div className="flex xl:flex-row flex-col gap-6 items-start">
              ...
              <div className="xl:hidden">
                <img
                  src={storyArcData?.smIcon}
                  loading="lazy"
                  className=" mx-auto"
                />
              </div>
              <div className="flex flex-col" key={storyArcData.tipsHeading}>
                <section>
                  <p className="text-2xl font-semibold text-text-500 font-poppins">
                    {' '}
                    {storyArcData.heading}
                  </p>
                  <p className="mt-[16px] text-text-200 text-base">
                    {storyArcData.paragrapgh}
                  </p>
                </section>
                <section className="mt-[24px]">
                  ...
                  <ul className="ml-2 mt-2">
                    {storyArcData.list.map((listItem) => {
                      return (
                        <>
                    ...
                    <Disclosure.Button className="rounded-[16px] border mb-10">
                      ...
                      {storyArcData && open &&
                        <section className="flex flex-col gap-y-3 p-4 h-fit overflow-auto font-poppins">
                          {storyArcData?.tips.map((item, index) => {
                            return (
                              <Disclosure.Panel
                                key={index}
...
export default WelcomeStoryArc;
