...
import { useDeleteLogline } from "../api/save-draft";
import { IoMdClose, IoMdRefresh, IoMdUndo } from 'react-icons/io';

import undoicon from '@/assets/refresh.svg'


const RemoveTemplatesModal = ({ setTemplatesPopup }: any) => {
  const { fileId } = useParams<{ type: string; fileId: string | "" }>();
  const deleteMutation = useDeleteLogline();
