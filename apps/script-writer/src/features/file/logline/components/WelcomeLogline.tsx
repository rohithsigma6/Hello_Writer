...
const WelcomeLogline = ({ setLoglineStatus }: { setLoglineStatus: (value: 'initialize') => void }) => {
  return (
    <>
        ...
        <div className="bg-white rounded-[24px] px-[32px] pt-[40px] font-poppins ">
          <div className="flex xl:flex-row flex-col gap-6 items-start">
            ...
            <div className="xl:hidden">
              <img
                src={loglineData?.smIcon}
                loading="lazy"
                className=" mx-auto"
              />
            </div>
            <div className="flex flex-col" key={loglineData.tipsHeading}>
              <section>
              ...
              <section className="gap-y-3">
                <button
                  ...
                  onClick={() => setLoglineStatus('initialize')}
                >
                  {loglineData.btnText}
                </button>
              </section>
                  ...
                  <Disclosure.Button className="rounded-[16px] border mb-10">
                    ...
                    {loglineData && open &&
                      <section className="flex flex-col gap-y-3 p-4 h-fit overflow-auto font-poppins">
                        {loglineData?.tips.map((item, index) => {
                          return (
                            <Disclosure.Panel
                              key={index}
